...
```bash
pip install -r requirements.txt        # Aliyun 镜像 + PyPI
python setup_wizard.py --self-test     # 写 .env + 预检 + 自检 + 预下载模型
python server.py                        # stdio 传输
```

`requirements.txt` 关键 pin：`fastembed>=0.7.0`（嵌入+重排共用，ONNX）、`onnxruntime==1.20.1`(py<3.13)/`1.21.1`(py≥3.13)、`setuptools<81`（APScheduler 3.x 依赖 pkg_resources）。**无 torch/transformers**——重排器走 ONNX，消除 Windows c10.dll/WinError 1114。
