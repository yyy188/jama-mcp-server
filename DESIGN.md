...
> 一份面向工程读者的高层 + 细节设计文档。基于对项目全部源码（约 6700 行 Python，13 个源文件）的逐文件阅读整理。
...
| 国内可用 | Aliyun pip 镜像 + HuggingFace 中国镜像（hf-mirror.com）+ CPU-only onnxruntime |
| 确定性安装 | uv 锁文件（uv.lock）锁定完整依赖树，`uv sync` 跨机复现一致环境 |
...
```
  ...
  Client    │  search_jama_semantics  query_jama_native_metadata           │
            │  bootstrap_models       get_sync_status   (+ 13 browse tools)│
            │                                                              │
            ...
            │      ├── net_guard.py     (下载前带宽预检 speed_test)        │
...
1. **MCP 服务层**（`server.py`，1766 行）：FastMCP 工具暴露、异步任务系统、APScheduler 增量同步、预检守卫、监控工具。
2. **RAG 流水线**（`rag_pipeline.py`，1011 行）：分块、嵌入提供者抽象、Multi-Query、混合召回、RRF、重排。
3. **Jama 客户端**（`jama_client.py`，989 行）：OAuth、并发分页拉取、HTML 清洗、browse API、原生元数据查询。
4. **存储层**（`db_setup.py`，574 行）：SQLite schema、FTS5 + sqlite-vec 加载、并发模型、CRUD。
5. **横切关注点**：`config.py`（462 行）、`preflight.py`（235 行）、`net_guard.py`（101 行）。
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `server.py` | 1766 | MCP 入口：23 个工具、异步任务池、增量同步调度、预检守卫、崩溃恢复 |
| `rag_pipeline.py` | 1011 | 分块(LlamaIndex)、嵌入(local/azure)、Multi-Query、混合召回、RRF、MiniLM 重排 |
| `jama_client.py` | 989 | OAuth2、并发分页、HTML→文本、browse API、原生查询、只读保证 |
| `config.py` | 462 | env→dataclass、校验、`.env` 持久化、热重载 |
| `db_setup.py` | 574 | SQLite schema、FTS5+vec0、WAL+写锁、原子替换、CRUD、维度变更重建 |
| `selftest.py` | 654 | 端到端自测套件 |
| `setup_wizard.py` | 222 | 交互式配置向导（写 .env + 预检 + 自检 + 预下载模型） |
| `net_guard.py` | 101 | 下载前带宽预检 `speed_test`（有界探针测吞吐，低于地板即 `NetworkTooSlowError`） |
| `preflight.py` | 235 | 离线依赖+配置+存储三段式校验 |
| `bootstrap.py` | 179 | 前台同步预下载嵌入+重排模型 |
...
- `RerankerSettings` — 默认 `cross-encoder/ms-marco-MiniLM-L-6-v2`（~80MB ONNX）。配置名是 torch 风格，`Reranker._resolve_model` 透明映射到 ONNX 版 `Xenova/ms-marco-MiniLM-L-6-v2`，经 fastembed/onnxruntime 加载——与 bge 嵌入共用同一 onnxruntime，**无 torch/transformers 依赖**（消除 Windows c10.dll/WinError 1114）。`allow_fallback=True`（加载失败降级为 RRF）。
...
chunks_vec USING vec0(chunk_id TEXT PRIMARY KEY, embedding FLOAT[{DIM}])
```
...
- 两者都支持 `item_type` 与 `modified_after/before`（经 `_normalize_iso_utc` 归一为定宽 UTC 字符串 `YYYY-MM-DDTHH:MM:SS.ffffff+0000` 做字典序范围比较）。**存储侧也对齐**：`_normalize_item`/`chunk_item` 在入库时用 `jama_client._normalize_date` 把 Jama 的 `modifiedDate`（`.000+0000`，3 位小数）归一为同样的 6 位小数定宽格式，否则边界点会比较失败——Jama 的 `.000+0000` 在小数位上 `+`(ASCII 43) < `0`(ASCII 48)，会比边界的 `.000000+0000` 字典序更小，导致 `modified_date >= after` 在「恰好等于边界」时误判为 False 而漏掉该条目。
...
- 在 `_normalize_item` 中清洗 `description` 与 `test_steps`，并拍平 `item_id/project_id/document_key/item_type/item_type_name/name/status/modified_date/created_date/raw_json`。`modified_date`/`created_date` 经 `_normalize_date` 归一为定宽 UTC（见 5.4）以保证范围比较正确。
...
### 7.2 重排器（交叉编码器，ONNX via fastembed）

`Reranker` 单例，默认 `cross-encoder/ms-marco-MiniLM-L-6-v2`（~80MB）。该 torch 风格名由 `_resolve_model()` **透明映射**到其 ONNX 移植版 `Xenova/ms-marco-MiniLM-L-6-v2`，用户 .env 无需改动即完成迁移：

- `_load()`：构造 `fastembed.TextCrossEncoder(model_name=<ONNX名>, cache_dir=..., cuda=False)` 并预热 `rerank("warmup", ["x"])`。失败设粘性 `_load_error` 返回 `False`。
- `_resolve_hf_repo()`：从 `TextCrossEncoder.list_supported_models()` 取真实 HF repo（缓存探测与测速探针用）。
- `_model_present()`：镜像 `LocalEmbeddingClient._model_present`，探测 `models--<org>--<name>` 下的 `.onnx` 文件（两种缓存布局）。
- `_download_model()`：镜像 `LocalEmbeddingClient._download_model`——`HF_ENDPOINT` 镜像顺序（hf-mirror.com → huggingface.co）+ `net_guard.speed_test` 预检 + `TextCrossEncoder(...)` 构造触发下载。`_download_lock` 双检。
- `rerank`：调 `self._model.rerank(query, texts)` 返回 **raw logit 分数**（可负；未做 sigmoid，因为只需相对排序）。
- **降级**：`rerank` 任何失败返回全零；`search` 用 `used_rerank = any(s!=0.0)` 检测，False 时改用 RRF 分数并标 `strategy="rrf"`。
- **保留契约**（6 个消费者依赖）：`_model`/`_load_error`/`ensure_downloaded(progress_callback=)`/`weights_cached()`/`rerank(query, texts, batch_size=None) -> list[float]`/`strategy ∈ {"rerank","rrf"}`。

> 设计说明：重排器与 bge 嵌入**共用 onnxruntime**（同一运行时，已在 Windows 验证干净加载），**完全移除 torch/transformers 依赖**，根除 `c10.dll`/WinError 1114 加载失败。MiniLM 交叉编码器用 sequence-classification head 直接打分 (query,doc) 对，无因果 LM 的大 logits 张量；RRF 已融合召回，重排只重排最终 25 条，小模型精度足够。
...
6. 传输选择 + `mcp.run(transport=...)` — 读 `JAMA_MCP_TRANSPORT`（默认 `stdio`；`streamable-http`/`sse` 时动态设 `mcp.settings.host`/`port`），阻塞服务。支持三种传输：stdio（本地子进程）、streamable-http（MCP 新标准，`/mcp` 路径）、sse（老客户端，`/sse` 路径）。`FASTMCP_HOST`/`FASTMCP_PORT` 环境变量不生效（构造函数 kwargs 优先），故 host/port 在 run 前直接赋值给 `mcp.settings`。
...
`_incremental_sync_all()`：`list_initialized_projects`（**仅 `READY`**——`INITIALIZING` 故意排除，避免调度同步与 `_resume_interrupted_syncs` 在 per-project lock 上竞态；卡死项目由启动恢复处理）→ 每个项目在 `_init_lock` 下查 active job，有则跳过，无则 `create_job(kind="sync")` 提交 `_run_job(..., incremental=True)`。增量逻辑在 `_sync_project_locked`：读 `last_sync_time` 作 `modified_after` 传给 `iter_project_items`，重分块/重嵌入/`replace_chunks` 原子替换，成功后推进 `last_sync_time=utcnow_iso()`，`max_items` 封顶。
...
`query_jama_native_metadata`：守卫 `{"jama"}` → 校验 `project_id` 数字、`item_type`（数字字符串，转 int；非法返回错误，与 `search_jama_semantics` 入参类型一致）→ 转 `jama().query_items_native(...,limit=20)`，返回 `{project_id,count,results}`。
...
模型（嵌入 ~130MB + 重排 ~80MB）**不打包**，首次使用下载。三入口：
...
- 调用方：`jama_client.preflight_speed_check`（同步前对 Jama `/projects` 首页测速）、`rag_pipeline.LocalEmbeddingClient._download_model`（对 HF 镜像 `config.json` 测速）。
- 重排模型权重走 `huggingface_hub.snapshot_download`（自带重试/超时）；其失败由 `Reranker._ensure_weights_downloaded` 包装成 `NetworkTooSlowError`。本模块已不再包含断点续传下载器（旧的 `download_with_retry` 已随 Qwen3 重排器迁移一并移除）。
...
1. **Python 包**：核心（requests/bs4/sqlite_vec/apscheduler + 按提供者的 fastembed，local 下 fastembed 同时服务嵌入与重排）缺失即 blocking；可选（mcp/llama_index）缺失仅 warn。
...
| 重排器 torch→fastembed ONNX 迁移 | 与 bge 嵌入共用 onnxruntime，移除 torch ~200MB 依赖；根除 Windows c10.dll/WinError 1114；透明映射旧模型名，用户 .env 无需改动 |
| onnxruntime pin（1.20.1/1.21.1） | 1.27.0（fastembed 0.8.0 在 py3.13 拉取）触发 VC++ DLL 问题；pin 到满足 fastembed 约束的最低稳定版 |
...
uv sync                                  # 从 uv.lock 创建 .venv + 装锁定依赖
cp .env.example .env                     # 填入 Jama 凭据
uv run python bootstrap.py               # 预下载模型 (~150MB ONNX)
uv run python server.py                  # stdio 传输（默认）
```

`uv.lock` 锁定完整依赖树（116 包含传递依赖），新机 `uv sync` 复现与开发机完全一致的环境，无版本解析漂移。备选：`pip install -r requirements.txt`。

`pyproject.toml` 关键 pin：`fastembed>=0.7.0`（嵌入+重排共用，ONNX）、`onnxruntime==1.20.1`(py<3.13)/`1.21.1`(py≥3.13)、`setuptools<81`（APScheduler 3.x 依赖 pkg_resources）。**无 torch/transformers**——重排器走 ONNX。纯 CPU（fastembed 构造强制 `cuda=False`）。

### 传输模式

`JAMA_MCP_TRANSPORT` 环境变量选择传输：

| 传输 | 用途 | 客户端连接 |
|------|------|-----------|
| `stdio`（默认） | 本地 MCP 客户端（Claude Desktop）以子进程启动 | stdin/stdout |
| `streamable-http` | 远程客户端 / 共享服务器 | `http://host:8000/mcp` |
| `sse` | 旧版 MCP 客户端 | `http://host:8000/sse` |

HTTP/SSE 模式下设 `JAMA_MCP_HOST=0.0.0.0`（远程访问）、`JAMA_MCP_PORT=8000`。FastMCP 原生支持三种传输，`mcp.settings.host/port` 在 `run()` 前动态设置（`FASTMCP_*` 环境变量因构造函数 kwargs 优先而不生效）。
...
├── pyproject.toml       # uv 项目定义 + 依赖（canonical）
├── uv.lock              # uv 锁文件（完整依赖树，确定性复现）
├── requirements.txt     # pip 备选安装（与 pyproject.toml 同步）
├── .env.example         # 配置模板
...
| `RERANKER_MODEL` | `cross-encoder/ms-marco-MiniLM-L-6-v2` | 重排器（透明映射到 ONNX 版） |
...
| `JAMA_MCP_TRANSPORT` | `stdio` | 传输：`stdio`/`streamable-http`/`sse` |
| `JAMA_MCP_HOST` / `JAMA_MCP_PORT` | `127.0.0.1` / `8000` | HTTP/SSE 监听地址（`0.0.0.0` 远程） |
